class EconomySimulation:
    ...
    def apply_policy(self, stimulus_amount: float):
        ...
        if stimulus_amount > 0:
            ...
            # 재정 승수 조정: 소비 대체 효과(기존 현금 대신 쿠폰 사용)를 고려하여 0.5로 하향
            ...
            # [수정] 쾌락 적응 (Hedonic Adaptation) 구현
    ...
    def calculate_market_reaction(self):
        ...
        current_growth = potential_growth - rate_drag - debt_drag
        ...
        # 통화량이 잠재성장률보다 빠르게 늘면 인플레 압력 (화폐수량설)
        excess_money_growth = money_growth_rate - potential_growth

        # 실물적 요인: GDP Gap 효과 (필립스 곡선 반영)
        # 실제 성장률이 잠재 성장률을 초과하면 수요 견인 인플레이션(Demand-Pull Inflation) 발생
        gdp_gap = self.gdp_growth_rate - potential_growth
        phillips_curve_effect = gdp_gap * 0.3

        # 금리 효과: 중립 금리(2.0%)보다 높으면 물가 억제 효과
        rate_effect = (self.interest_rate - 2.0) * 0.8
        
        # 신규 인플레이션 압력 계산
        # 기본 2.0% + 초과 통화량 + GDP Gap(필립스 곡선) + 단기 유통속도 충격 - 금리 효과
        target_inflation = 2.0 + (excess_money_growth * 0.5) + phillips_curve_effect + self.stimulus_velocity_impact - rate_effect
        
        # 인플레이션: 관성(0.8) + 신규 압력(0.2)
        # 수정: 물가는 경직성(Stickiness)이 강함. 이전 분기 물가의 영향을 80%로 높여 급격한 변동 방지
        ...
        # [수정] 부채 패널티 강화: 선형(-)이 아니라 제곱(--)으로 패널티를 주어, 부채가 쌓일수록 공포감 조성
        ...
        # 금리가 2.0%를 초과하면 이자 상환 부담으로 행복도가 급격히 하락 (DSR 부담 증가)
        if self.interest_rate > 2.0:
            # 가계 부채 민감도 강화: 제곱항 계수를 1.0 -> 1.5로 상향하여 이자 부담 현실화
